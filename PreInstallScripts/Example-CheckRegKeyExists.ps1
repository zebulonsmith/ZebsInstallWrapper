...
They should all return an instance of the [ScriptResult] class defined in SoftwareInstallHelperFunctions.ps1.
THis example checks to see if $regKey exists.
...
Write-Logfile -loglevel Information -path $logfile "Evaluating the existence of $($regkey)"
Try {
...
if ( $RegKeyExists ) {#The directory exists, return a success
    Write-LogFile -LogLevel Information -Path $logfile -Message "Condition evaluated True."
    $result = "SUCCESS"
    $resultDetails = "$($regKey) exists."
    ...
    $result = "ERROR"
    $resultDetails = "$($regKey) does not exist."
}
#endregion
...
$PreInstallResult.Description = "Evaluate whether or not $($regKey) exists" #Describe what we're doing here
